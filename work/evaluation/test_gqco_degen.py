...
group.add_argument('--degree', type=int, default=2, help='Degree od degeneracy')
...
for itr in tqdm.tqdm(range(len(testdata[size])), desc=f'temperature: {temperature}, clone: {num_clone}, size: {size}'):
    ...
    #         # energy, obsv = task.compute_energy(t, adj, args.num_shot)
    #         energy = task.compute_energy(t, adj, args.num_shot)
    #         energy_cache[t_tuple] = energy
    #         # vector_cache[t_tuple] = obsv
    #     else:
    ...
    # energy_min = energy_cache[tuple(token_min)]
    # obs_min = vector_cache[tuple(token_min)]
...
with open(f'./outputs/gqcoans_s{SEED}_t{temperature}_cl{num_clone}_deg{degree}.pkl', 'wb') as f:
    pickle.dump({
        'answer': _ans,
        'time': _tms,
        'tokens': _tokens
